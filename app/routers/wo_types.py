...
async def list_wo_types(project_id: int, user: ClerkUser = Depends(get_current_user)):
...
async def create_wo_type(project_id: int, body: WOTypeCreate, user: ClerkUser = Depends(get_current_user)):
    return await WOType.create(project_id=project_id, **body.model_dump())


@router.patch("/wo-types/{type_id}", response_model=WOTypeOut)
async def update_wo_type(type_id: int, body: WOTypeUpdate, user: ClerkUser = Depends(get_current_user)):
...
async def delete_wo_type(type_id: int, user: ClerkUser = Depends(get_current_user)):
    wt = await WOType.get_or_none(id=type_id)
    if not wt:
        raise HTTPException(404, "WO type not found")
    await wt.delete()
